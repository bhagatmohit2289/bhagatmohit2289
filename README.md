# 👨‍💻 Mohit Bhagat

**Full Stack Developer | Technology Lead**

---

## Welcome to my GitHub profile!

Full Stack Developer with **11+ years of experience** in designing and developing high-performance, scalable web applications. Currently working as **Technology Lead at Infosys**, specializing in enterprise storage solutions and microservices architecture. Passionate about building robust systems, mentoring teams, and delivering impactful solutions.

**Location:** Hove, UK (Brighton) | **Phone:** +44 7825278531

---

## 👋 Connect with Me

- **Email:** [bhagat.mohit2289@gmail.com](mailto:bhagat.mohit2289@gmail.com)
- **LinkedIn:** [linkedin.com/in/mohitbhagat](https://www.linkedin.com/in/mohitbhagat)
- **Phone:** +44 7825278531
- **Address:** 29 Cliftonville Court, Hove, BN3 3RX, UK

---

## 💼 Professional Experience

### **Technology Lead** 👉 [Infosys](https://www.infosys.com/) | Amex, UK | 2019 - Present ✔️
- **Domain:** Enterprise Storage
- **Key Achievement:** Developed performance-oriented storage system for document ingestion
- **Team Size:** 10 members
- **Methodology:** Agile Based
- **Tech Stack:** Vert.X, Java, Couchbase DB, XLR, Splunk, S3, JIRA, Kafka, RTF

### **Senior Software Engineer** 👉 Banca Sella | Chennai, India | 2018 - 2019
- **Domain:** Banking
- **Responsibility:** Full Stack Developer - Built application from scratch
- **Team Size:** 3 members
- **Methodology:** Waterfall
- **Tech Stack:** ReactJS, Spring v4, JDBC, CSS, Java 1.7, Oracle, JUnit, UNIX, JIRA, Tomcat

### **Software Engineer** 👉 [FIS Global](https://www.fisglobal.com/) | Chennai, India & Germany | 2015 - 2018
- **Domain:** Banking
- **Responsibilities:** Enhanced existing application & built new features from scratch
- **International Exposure:** Germany
- **Team Size:** 5 members
- **Methodology:** Waterfall
- **Tech Stack:** Spring Boot, Hibernate 3, CSS, JDK 1.7, Oracle, JUnit, Swagger UI, JIRA, JSF2.2, PrimeFaces 3, EJB, JMS, JPA, JQuery, WebSphere, Tomcat, Active MQ

### **Software Engineer** 👉 [Syntel](https://www.sap.com/partner/profile/11651) | Chennai, India | 2012 - 2015
- **Domain:** Banking
- **Responsibility:** Enhanced and maintained banking applications
- **Team Size:** 5 members
- **Methodology:** Agile Based
- **Tech Stack:** Java, Spring, Hibernate, Manual Testing, Quality Centre, MySQL, WebSphere, JavaScript, HTML, CSS, JSP

---

## 👀 Tech Stack Overview

### **Primary Technologies**
- **Languages:** Java 1.8, Vert.X, Spring, Spring Boot, React, Redux, Javascript, Hibernate, JPA, JDBC, REST APIs, Log4j, JUnit

### **Frontend Development**
- **Languages:** JavaScript, TypeScript, HTML5, CSS3, JSP
- **Frameworks & Libraries:** React.js, Redux, jQuery, PrimeFaces 6
- **Styling:** CSS, Bootstrap, SCSS
- **Tools:** VS Code, Webpack, Swagger UI

### **Backend Development**
- **Languages:** Java 1.8, Java 1.7, Vert.X, Python
- **Frameworks:** Spring, Spring Boot, Spring v4, EJB 2.0-3.0, JSF 2.2
- **APIs:** REST APIs, JMS, Kafka, RTF, SOAP
- **Database Tools:** Hibernate, JPA, JDBC

### **Databases & Storage**
- **SQL:** Oracle, MySQL
- **NoSQL:** Couchbase, MongoDB
- **Cloud Storage:** Amazon S3, Google Cloud Storage

### **DevOps & Cloud**
- **Cloud Platforms:** AWS (S3, XLR, Splunk)
- **Servers:** Tomcat, Node.js, WebSphere, WebLogic, JBOSS, Active MQ
- **CI/CD:** Jenkins, XLR
- **Monitoring:** Splunk, JIRA

### **Tools & Platforms**
- **Version Control:** GIT, SVN, Clear Case
- **Build Tools:** Maven, Webpack
- **IDEs:** VS Code, Eclipse
- **Testing:** JUnit, Swagger UI
- **Monitoring & Logging:** Splunk, Log4j, XLR

### **Design Patterns**
- MVC, Singleton, Asynchronous, Factory, Facade

### **Programming Styles**
- Functional & Object Oriented Programming

---

## 🎓 Education

- **B-Tech. in Electronics and Communication Engineering (ECE)** 👉 2007 - 2011

---

## 📊 GitHub Statistics

- 💻 Experienced Full Stack Developer with enterprise-level expertise
- 🌱 Specialized in Java, Spring Boot, and microservices architecture
- 👋 Leader & mentor with proven team management skills
- 💡 Strong problem solver with international work experience
- 👾 DevOps & Cloud proficiency (AWS, Splunk, Jenkins)

---

## 🚀 Featured Projects

*Showcase your best work here*

1. **Enterprise Storage System** - Document Ingestion Platform
   - Technologies used: Vert.X, Java, Couchbase, Kafka, S3, Splunk
   - Key Features: High-performance document ingestion, monitoring, scalability

2. **Banking Application** - Full Stack Development
   - Technologies used: ReactJS, Spring Boot, Oracle, JSF
   - Key Features: Full feature implementation from scratch

---

## ⭐ Let's Collaborate!

I'm always interested in:
- 🔗 Building enterprise-scale applications
- 🌗 Mentoring junior developers
- 💻 Working on microservices and cloud solutions
- 👋 Contributing to impactful projects
- 🌐 Open-source contributions
- 💬 Technical discussions & knowledge sharing

Feel free to reach out if you'd like to collaborate or discuss tech!

---

*Thanks for visiting my profile! Don't forget to star ⭐ your favorite repositories!*
